...
class MyViewTests(TestCase):
    def setUp(self):
        ...
        self.menu_item1 = MenuItem.objects.create(
            name="Item 1", price=10.0, category=self.category, is_available=True
        )
        self.menu_item2 = MenuItem.objects.create(
            name="Item 2", price=15.0, category=self.category, is_available=False
        )
...
class CafeModelTests(TestCase):
    ...
    def test_cafe_creation(self):
        ...
        self.assertEqual(
            self.cafe.opening_time, time(8, 0)
        )  # Compare with datetime.time
        # Assert that the closing time is correctly set
        self.assertEqual(
            self.cafe.closing_time, time(20, 0)
        )  # Compare with datetime.time
    ...
    def test_is_closed(self):
        ...
        self.assertFalse(
            self.cafe.is_open(closed_time), "Cafe should be closed at 21:00"
        )
