...
from order.models import Order  # Import Order if it's used in the tests
from staff.models import Staff  # Import your custom user model
...
class CustomerModelTests(TestCase):
    ...
    def test_customer_string_representation(self):
        ...
        self.assertEqual(str(customer), 'Bob Brown')
...
class CustomerViewTests(TestCase):
    ...
    def test_customer_checkout_without_orders(self):
        ...
        self.assertEqual(len(response.context['orders']), 0)  # Ensure no orders in the context

    def test_search_customer_as_staff(self):
        """Test the search_customer view as a staff user."""
        response = self.client.get(reverse('search_customer'), {'phone_number': self.customer.phone_number})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['customers']), 1)  # Ensure we found the customer
    ...
    def test_search_customer_as_non_staff(self):
        ...
        self.assertIn('/login/', response.url)  # Check if it redirects to the login URL

    def test_search_customer_with_no_results(self):
        """Test the search_customer view with no matching customers."""
        response = self.client.get(reverse('search_customer'), {'phone_number': '123456789'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['customers']), 0)  # Ensure no customers found
